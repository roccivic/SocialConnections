...
public class FacialRecognition {
	 ...
	 */
	public synchronized void upload(final String session, final String path, final int numFaces, final List<Integer> ignored) {
		new Thread(
			new Runnable() {
				public void run() {
					__upload(session, path, numFaces, ignored);
				}
			}
		).start();
	}
	 ...
	 */
	private void __upload(String session, String path, final int numFaces, List<Integer> ignored) {
		...
		try {
			...
			for (int i=0; i<numFaces; i++) {
				if (! ignored.contains(i)) {
					String filename = path + "face" + i + ".jpg";
					pairs.addPart("image[]",  new FileBody(new File(filename), "image/jpeg"));
				}
			}
