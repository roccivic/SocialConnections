...
public class FacialRecognition {
	 ...
	 */
	public synchronized void upload(final String session, final String path, final int numFaces, final int offset) {
		new Thread(
			new Runnable() {
				public void run() {
					__upload(session, path, numFaces, offset);
				}
			}
		).start();
	}
	 ...
	 */
	private void __upload(String session, String path, final int numFaces, final int offset) {
		...
		try {
			...
			for (int i=0; i<numFaces; i++) {
				String filename = path + "face" + (offset + i) + ".jpg";
